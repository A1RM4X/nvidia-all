...
if [ -z "$_driver_version" ] || [ "$_driver_version" = "latest" ] || [ -z "$_driver_branch" ] && [ ! -e options ]; then
  ...
  if [[ -z $CONDITION ]]; then
    ...
    if [ "$CONDITION" = "2" ]; then
      echo '_driver_version=550.54.14' > options
      echo '_md5sum=69e77647875bef192f8458912027e44b' >> options
    ...
    # This (condition 1) will be treated as the latest Vulkan developer driver.
      ...
      echo '_driver_version=535.43.28' > options
      echo '_md5sum=6b37f13403856e6b8176b5db4fa8b47e' >> options
      echo '_driver_branch=vulkandev' >> options
    fi
# Package type selector
  if [ -z "$_dkms" ]; then
